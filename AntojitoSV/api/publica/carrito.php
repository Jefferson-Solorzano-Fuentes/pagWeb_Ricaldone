...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'updateQuantity':
            ...
            } elseif (!$detalle_pedido->setSubtotal($_POST['subTotalDetallePedido'])) {
                $result[EXCEPTION] = 'id incorrecto';
        ...
        if (!$pedido->setDireccion($_POST['direccion'])) {
            $result[EXCEPTION] = 'direccion no correcta';
        } elseif (!$pedido -> setDescripcion(($_POST['descripcion']))) {
            $result[EXCEPTION] = 'descripcion no correcta';
        } elseif (!$pedido -> setCreacion(($_POST['fecha_creacion']))) {
            $result[EXCEPTION] = 'fecha no correcta';
        }  elseif (!$pedido -> setMonto(($_POST['total']))) {
            $result[EXCEPTION] = 'total no creado';
        }  else if ($pedido->createRow()) {
            $result[STATUS] = SUCESS_RESPONSE;
            $result[MESSAGE] = ' pedido actualizado existosamente';
            if ($result[DATA_SET] = $detalle_pedido->readAll()) {
                $result[STATUS] = SUCESS_RESPONSE;
            } else {
                $result[EXCEPTION] = 'No hay datos registrados';
